...
def plot_gantt(df, selected_machine):
    ...
    num_jobs = len(machine_df)  # Anzahl der Jobs in der Maschine

    for index, row in machine_df.iterrows():
        # Plotten des Balkens mit y=1
        plt.barh(1, row['Processing Time'], left=row['Start'], edgecolor='black', align='center')

        # Platzieren des Textes innerhalb des Balkens
        text_x_position = row['Start'] + row['Processing Time'] / 2  # Mittelpunkt des Balkens
        plt.text(text_x_position, 1, f"Job {row['Job']} ALT {row['Alternative']} OP {row['Operation']}", 
    ...
    # Festlegen der y-Achsenticks auf [1] mit Beschriftung 'Job'
    plt.yticks([1], ['Job'])
